...
if __name__ == "__main__":
    ...
    comment_cnt = 0
    vote_cnt = 0
    for op in b.stream(start=int(start_block), stop=int(end_block), opNames=["vote", "comment"], threading=False,
                       thread_num=8):
        ...
        if "trx_num" in op:
            ...
            trx_num = 0
        data = {"block_num": block_num, "block_id": op["_id"], "trx_id": op["trx_id"], "trx_num": trx_num,
                "op_num": op_num, "timestamp": formatTimeString(op["timestamp"]), "type": op["type"]}
        ...
        if op["type"] == "comment":
            ...
            if op["author"] in member_accounts and op["voter"] in accounts:
                ...
                curation_data = {"authorperm": authorperm, "member": op["author"], "created": c["created"],
                                 "best_time_delay": best_time_delay, "best_curation_performance": best_performance,
                                 "vote_rshares": int(vote["rshares"]), "updated": datetime.utcnow(),
        ...
        if cnt % 200 == 0 and cnt > 0:
            ...
            print("Duration for %d blocks: %.2f s (%.3f s per block) -- %.2f hours to go" % (
            block_diff_for_db_storage, time_for_blocks, time_for_blocks / block_diff_for_db_storage, running_hours))
