...
if __name__ == "__main__":
    ...
    posts_dict = {}
    changed_member_data = []
    for ops in b.stream(start=start_block, stop=stop_block, opNames=["comment"], max_batch_size=max_batch_size,
                        threading=threading, thread_num=8):
        ...
        if main_post:
            ...
            if status_command > -1 and abs((ops["timestamp"] - c["created"]).total_seconds()) <= 10:
                ...
                rshares_denom = member_data[ops["author"]]["rewarded_rshares"] + member_data[ops["author"]][
                    "balance_rshares"]
                ...
                try:
                    stm.post("", reply_body, app="steembasicincome/%s" % sbiversion, author=account_name,
        ...
        if vote_delay is None:
            vote_delay = 300
        posts_dict[authorperm] = {"authorperm": authorperm, "author": ops["author"], "created": dt_created,
                                  "block": ops["block_num"], "main_post": main_post,
